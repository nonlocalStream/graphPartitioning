...
*)
	...
	Options[MultipleFiedlerVector]={(*Range->{1/5,4/5}, *)NumEigen->5, Basis->SpectralVectors}
	MultipleFiedlerVector[g_Graph, OptionsPattern[]] := With[
		{basisVectors=OptionValue[Basis][g,OptionValue[NumEigen]]},
			VectorPartition[basisVectors, V[g], OptimizationOn->True,
				OptimizationObj-> (CriterionPartitionFunction[g,#,0.5]&)]]

	Options[MultipleIsoperimetricVector]={NumBasis->5, xi->Eigenvalues[N[LaplacianMatrix[g]]][[1]]}
	MultipleIsoperimetricVector[g_Graph, OptionsPattern[]] := 
		MultipleFiedlerVector[g, NumBasis->OptionValue[NumBasis],
			Basis->(FingerprintVectors[#1,#2,OptionValue[xi]]&)]
	(*Maximum[
	...
	Options[FingerprintVectors]={Epsilon->10^-6}
	FingerprintVectors[g_Graph, k_Integer, xi_, OptionsPattern[]] := With[
		...
		L=N[LaplacianMatrix[g]], epsilonI = OptionValue[Epsilon]*IdentityMatrix[k]},
			With[{J = Transpose[B].(xi*M-L).B},
				Print[MatrixForm[B]];
				Transpose[CholeskyDecomposition[J+epsilonI].Transpose[B].M]]]
